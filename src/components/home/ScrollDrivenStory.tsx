...
const stickyViewportFrameClassName =
  "sticky top-[5.75rem] h-[calc(100svh-5.75rem)] self-start";
...
/**
 * Keeps the chapter activation line comfortable on both short laptops
 * and taller desktop displays.
 */
function getActivationPoint(viewportHeight: number) {
  return Math.min(Math.max(viewportHeight * 0.46, 260), 460);
}
...
export function ScrollDrivenStory() {
  ...
  useEffect(() => {
    ...
    const updateActiveScene = () => {
      ...
      const activationPoint = getActivationPoint(window.innerHeight);

      /*
       * Scene 01 remains active until the top of Scene 02 crosses the
       * activation line. The same rule applies to every following scene.
       *
       * This is more stable than comparing changing intersection ratios and
       * prevents flickering when two large chapters overlap the viewport.
       */
      let nextActive = 0;

      for (let index = 1; index < sceneRefs.current.length; index += 1) {
    ...
    const requestActiveSceneUpdate = () => {
      ...
      animationFrameId = window.requestAnimationFrame(updateActiveScene);
    };
    ...
    const syncSceneObserver = () => {
      ...
      sceneObserver = new IntersectionObserver(requestActiveSceneUpdate, {
    ...
    window.addEventListener("scroll", requestActiveSceneUpdate, {
      passive: true,
    });

    document.addEventListener("scroll", requestActiveSceneUpdate, {
      capture: true,
      passive: true,
    });
    ...
    return () => {
      window.removeEventListener("scroll", requestActiveSceneUpdate);
      document.removeEventListener("scroll", requestActiveSceneUpdate, true);
  ...
  const scrollToScene = (index: number) => {
    ...
    const activationPoint = getActivationPoint(window.innerHeight);
    const nodeTop = window.scrollY + node.getBoundingClientRect().top;

    const targetScrollTop = Math.max(
      0,
      Math.round(nodeTop - activationPoint + 1),
    );
  ...
  return (
    ...
    >
      {/* ------------------------------------------------------------------ */}
      {/* Desktop and laptop cinematic layout                                */}
      {/* ------------------------------------------------------------------ */}

      <div className="mx-auto hidden w-full max-w-[1600px] lg:grid lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] lg:items-stretch">
        {/*
         * This wrapper stretches to the full natural height created by the
         * right column. The sticky panel inside it is therefore constrained
         * to the story section and releases when this wrapper ends.
         */}
        <div className="relative min-w-0">
          <div className={cn(stickyViewportFrameClassName, "w-full")}>
            {/*
             * overflow-hidden is intentionally limited to the image frame.
             * The sticky element and all of its ancestors remain unclipped.
             */}
            <div className="relative h-full w-full overflow-hidden lg:rounded-tl-[3.5rem]">
              {scenes.map((scene, index) => {
                ...
                return (
                  <Image
                    ...
                    src={scene.image}
                    alt={
                      isActive ? `${featuredStory.title}: ${scene.title}` : ""
                    }
                    fill
                    priority={index === 0}
                    ...
                    className={cn(
                      ...
                      isActive && "translate-x-0 scale-100 opacity-100",
                      !isActive &&
                        isPrevious &&
                ...
                {scenes.map((scene, index) => (
                  <p
                    ...
                    className={cn(
                      ...
                      "text-brand-ivory/70 transition-[opacity,transform] duration-500",
                      "motion-reduce:transform-none motion-reduce:transition-none",
                      active === index
              ...
              {/* Image progress and story counter */}
        ...
        {/* Normal document-flow story content */}
        ...
        <div className="min-w-0 px-8 pb-16 pt-16 xl:px-10 xl:pb-20 xl:pt-20 2xl:px-14 2xl:pb-24 2xl:pt-24">
          <header className="mb-12 max-w-xl xl:mb-16">
            ...
            <h2 className="display-heading mt-5 break-words text-[clamp(3.25rem,4.7vw,6rem)]">
              ...
              <span className="block">part of the day.</span>
            </h2>

            <p className="mt-6 max-w-lg text-base leading-7 text-brand-ivory/70 xl:mt-7 xl:text-lg xl:leading-8">
              Follow one Elegant Star invitation from its first printed detail
              to the celebration it was created for.
            </p>
          </header>

          <div className="grid min-w-0 grid-cols-[2.75rem_minmax(0,1fr)] items-stretch gap-6 xl:grid-cols-[3rem_minmax(0,1fr)] xl:gap-8">
            {/*
             * A full-viewport sticky navigation frame keeps the chapter
             * markers vertically centred even on 720–768px-high displays.
             */}
            <nav
              ...
              {scenes.map((scene, index) => {
                ...
                return (
                  ...
                  >
                    <article
                      className={cn(
                        "min-w-0 max-w-xl transition-[opacity,transform] duration-500",
                        "motion-reduce:transform-none motion-reduce:transition-none",
                        isActive
                          ? "translate-y-0 opacity-100"
                    ...
                    >
                      <p
                        ...
                        style={{
                          transitionDelay: isActive ? "40ms" : "0ms",
                        }}
                      >
                        {scene.label}
                      </p>
                      ...
                      {isFinalScene ? (
                        <div
                          ...
                          style={{
                            transitionDelay: isActive ? "190ms" : "0ms",
                          }}
                        >
                          <Link
      ...
      <div className="w-full max-w-full lg:hidden">
        <header className="px-5 pb-12 pt-16 sm:pb-14 sm:pt-20">
          <p className="small-label text-brand-champagne">
            From stationery to celebration
          </p>

          <h2 className="display-heading mt-4 break-words text-[2.75rem] leading-[0.98] sm:text-6xl">
            A design that became <span className="block">part of the day.</span>
          </h2>

          <p className="mt-6 max-w-md text-base leading-7 text-brand-ivory/70">
            Follow one Elegant Star invitation from its first printed detail to
            the celebration it was created for.
          </p>
        </header>
        ...
        <div className="w-full max-w-full">
          {scenes.map((scene, index) => {
            ...
            return (
              <article
                key={scene.id}
                className="w-full max-w-full border-t border-brand-ivory/10 py-10 first:border-t-0 first:pt-0 sm:py-12"
              >
                <div className="relative mx-5 aspect-[4/5] overflow-hidden rounded-[1.35rem] sm:aspect-[5/6] sm:rounded-[1.75rem]">
                  <Image
                    src={scene.image}
                    alt={`${featuredStory.title}: ${scene.title}`}
                    fill
                    sizes="(max-width: 1024px) calc(100vw - 2.5rem), 50vw"
                    style={{
                      objectPosition: scene.objectPosition,
                    }}
                    className="object-cover transition-transform duration-700 motion-reduce:transform-none motion-reduce:transition-none"
                  />

                  <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-brand-olive/70 via-transparent to-black/10" />

                  <div className="absolute left-4 top-4 flex h-10 min-w-10 items-center justify-center rounded-full border border-brand-ivory/20 bg-brand-olive/[0.48] px-3 text-xs font-bold tabular-nums backdrop-blur-xl">
                    {scene.number}
                  </div>

                  <p className="small-label absolute bottom-5 left-5 text-brand-ivory/75">
                    {scene.imageCaption}
                  </p>
                </div>

                <div className="min-w-0 px-5 pt-7">
                  <p className="small-label text-brand-champagne">
                    {scene.label}
                  </p>

                  <h3 className="mt-4 max-w-md break-words font-display text-[2.65rem] leading-[0.98] sm:text-6xl">
                    {scene.title}
                  </h3>

                  <p className="mt-5 max-w-lg text-base leading-7 text-brand-ivory/70">
                    {scene.text}
                  </p>
                    ...
                    <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                      <Link
                        ...
                        className={cn(
                          ...
                          "rounded-full bg-brand-ivory px-5 py-3.5",
